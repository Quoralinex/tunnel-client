...
func TestHelpTopicQuickstart(t *testing.T) {
	...
	require.Contains(t, stdout, "tunnel-client run --embedded-stateless-mcp-stub")
	require.Contains(t, stdout, "no MCP session ID, even for initialize and notifications/initialized")
	require.Contains(t, stdout, "Stateless demo tools need no MCP session affinity")
	require.Contains(t, stdout, "--embedded-mcp-listen-addr (127.0.0.1:0)")
